...
| **build** | Kaniko push to `$CI_REGISTRY_IMAGE` (no DinD) |
...
| `build.yml` | `build:kaniko` | Tags: `$AEGISML_IMAGE_NAME-$CI_COMMIT_SHORT_SHA`, `$AEGISML_IMAGE_NAME-latest` |
...
| `policy_check.yml` | `policy_check:ai-devsecops` | Runs when `POLICY_AGENT_PACKAGE` is set; fails on high/critical |
...
On each run (when enabled), see **`artifacts/policy-check/`**:
